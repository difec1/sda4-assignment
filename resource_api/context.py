...
def evaluate_request_context(claims: dict, path: str, method: str) -> Decision:
    ...
    if path in SENSITIVE_PATHS: #Updated by Group D: This Rule should only apply to all SENSITIVE_PATHS routes
      #Added by Group D: Role name is administrator and clearance_level must be "Restricted" so thath only root can access (Task 1; Sensitive endpoints with stricter checks)
      if role == "administrator" and clearance_level == "Restricted": 
        return "allow"
      else:
        return "challenge"
